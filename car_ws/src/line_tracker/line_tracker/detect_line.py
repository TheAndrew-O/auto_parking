...
class DetectLine(Node):
    def __init__(self):
        ...
        self.left_image = self.create_subscription(Image,"/left_camera_sensor/image_raw",self.turn_left_callback,rclpy.qos.QoSPresetProfiles.SENSOR_DATA.value)
        self.right_image = self.create_subscription(Image,"/right_camera_sensor/image_raw",self.turn_right_callback,rclpy.qos.QoSPresetProfiles.SENSOR_DATA.value)
        ...
        self.right_image_out = self.create_publisher(Image,"/right_image_out",1)
        self.line_pub  = self.create_publisher(Point,"/detected_lines",1)
        self.turn_left_pub = self.create_publisher(String, "/turn_left", 1)
        self.turn_right_pub = self.create_publisher(String, "/turn_right", 1)
        ...
        self.declare_parameter("h_min",45)
        self.declare_parameter("h_max",69)
        self.declare_parameter("s_min",174)
        self.declare_parameter("s_max",255)
        self.declare_parameter("v_min",0)
        self.declare_parameter("v_max",255)
        self.declare_parameter("sz_min",0)
        self.declare_parameter("sz_max",100)
        self.declare_parameter("x_min2",0)
        self.declare_parameter("x_max2",85)
        self.declare_parameter("y_min2",35)
        self.declare_parameter("y_max2",100)
        self.declare_parameter("h_min2",45)
        self.declare_parameter("h_max2",69)
        self.declare_parameter("s_min2",174)
        ...
        self.declare_parameter("x_min_right",15)
        ...
        self.tuning_params3 = {
            'x_min': self.get_parameter('x_min_right').get_parameter_value().integer_value,
            'x_max': self.get_parameter('x_max').get_parameter_value().integer_value,
            'y_min': self.get_parameter('y_min2').get_parameter_value().integer_value,
            'y_max': self.get_parameter('y_max2').get_parameter_value().integer_value,
            'h_min': self.get_parameter('h_min2').get_parameter_value().integer_value,
    ...
    def turn_left_callback(self, data):
        try:
            cv_image = self.bridge.imgmsg_to_cv2(data, "bgr8")
        except CvBridgeError as e:
            print(e)
    ...
    def turn_right_callback(self, data):
        try:
            cv_image = self.bridge.imgmsg_to_cv2(data, "bgr8")
        except CvBridgeError as e:
            print(e)
        ...
        try:
            ...
            if lines is not None:
                ...
                if(point.z > 0):
                    msg.data = 'right'
                    self.turn_right_pub.publish(msg)
                else:
                    msg.data = 'noturn'
                    self.turn_right_pub.publish(msg)
